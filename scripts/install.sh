...
resolve_command_paths() {
    ...
    SG_SYNC_PATH="$(find_cmd_path sg_sync)"
    SG_LOGS_PATH="$(find_cmd_path sg_logs)"
    ...
    cat > "$CONFIG_DIR/command_paths.json" << EOF
{
  ...
  "sg_inq": "$SG_INQ_PATH",
  "sg_sync": "$SG_SYNC_PATH",
  "sg_logs": "$SG_LOGS_PATH",
  "dd": "$DD_PATH",
  "lsblk": "$LSBLK_PATH",
  "lshw": "$LSHW_PATH",
...
prompt_interactive_config() {
    ...
    if [ -t 0 ]; then
        ...
        # 6. Prompt for Network Access Mode
        echo -e "  Network Access Mode controls which machines can connect to this station."
        echo -e "    [1] Local only (127.0.0.1) — no other machine can connect"
        echo -e "    [2] Remote allowed (0.0.0.0) — LAN access with authentication"
        echo -e -n "  Select mode [Default: 2]: "
        local access_mode
        read -r access_mode
        if [ "$access_mode" = "1" ]; then
            BIND_ADDRESS="127.0.0.1"
            success "Network access restricted to localhost only."
        else
            BIND_ADDRESS="0.0.0.0"
            success "Remote network access allowed (authentication required)."
        fi
        echo ""
...
setup_config() {
    ...
    if [ ! -f "$CONFIG_DIR/bay_map.json" ]; then
        info "Creating default empty bay_map.json..."
        echo '{}' > "$CONFIG_DIR/bay_map.json"
        success "Empty bay_map.json created."
    else
...
data = {
  ...
  'background_smart_max_workers': 8,
  'max_concurrent_wipes': 34,
  'discovery_diag': False,
  'log_retention_days': 30,
  'max_logo_size_mb': 1,
  'max_bulk_cert_batch_size': 100,
  ...
  'zero_check_timeout_seconds': 30,
  'zero_check_startup_delay_seconds': 30,
...
Cmnd_Alias DISK_CMDS = $SMARTCTL_PATH, $HDPARM_PATH, $NVME_PATH, $SG_SANITIZE_PATH, $SG_INQ_PATH, $SG_SYNC_PATH, $SG_LOGS_PATH, $DD_PATH, $LSBLK_PATH, $LSHW_PATH, $SYSTEMCTL_PATH, $BLOCKDEV_PATH
