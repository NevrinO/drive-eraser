...
### 102. Coordinate In-Flight Background Tasks with Cache Invalidation
- **Rule**: When a background task reads from a cache, performs long-running work, and then writes back to the cache, the cache invalidation mechanism must also cancel or invalidate the in-flight work.
- **Guardrail**: A cache invalidation event (e.g., drive hot-plug, passphrase change, bay mapping update) must not only clear the cache entry but also clear any pending/in-flight tracker for that key. Otherwise, a long-running task that started before invalidation will still write stale data when it completes. Use a generation counter or version per cache entry: the task snapshots the generation when it starts and only writes if the generation has not advanced. Alternatively, maintain a pending set that is cleared atomically with the cache invalidation. This prevents stale data from being served for the cache TTL after invalidation.
...
### 106. UI Controls Must Match Backend Policy Semantics
- **Rule**: When a frontend control mirrors or defaults from a backend policy/configuration value, the control must accurately represent what the backend will actually do, not just reuse the same option names.
- **Guardrail**: A backend may have layered controls (e.g., a global policy switch and a per-request parameter) that affect different code paths. Before building a frontend dropdown or toggle that defaults from a policy value, trace the backend logic to confirm which paths the policy controls and which paths the per-request parameter controls. If the frontend conflates the two layers, the UI will mislead operators about the actual behavior. In the case of a global policy that disables some but not all verification paths, the UI must either reflect that scope or extend the backend API to accept a per-request mode that the backend honors consistently.

### 107. Synchronize Validation Logic Changes with Tests and Comments
- **Rule**: When changing validation logic (regex patterns, whitelist rules, schema constraints), you must simultaneously update all corresponding tests, assertions, and inline comments to reflect the new behavior.
- **Guardrail**: A validation change that makes a previously optional field mandatory (or vice versa) must be reflected in: (1) the actual validation regex/schema, (2) test assertions that verify valid/invalid inputs, and (3) comments that document the validation rules. A mismatch between implementation and tests creates false confidence—tests may pass with the old logic but fail with the new, or vice versa. Before committing a validation change, read the test file to ensure all assertions and comments accurately describe the new behavior. If the change intentionally breaks backward compatibility (e.g., rejecting previously-valid inputs), update the test expectations and document the breaking change.
