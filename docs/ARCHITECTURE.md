...
## Unmapped Drives Endpoint Performance
- **Current decision (Option A)**: `/api/admin/unmapped-drives` uses `smartctl -j -i` (identity-only) plus a 10-minute per-device cache, instead of `smartctl -j -x` (full extended SMART). The endpoint only needs model, serial, and capacity, so full SMART was unnecessary and caused 60-second page loads on systems with many unmapped drives.
